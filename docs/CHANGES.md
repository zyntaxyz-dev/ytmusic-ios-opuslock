...
| 2026-09-04 | fix-domain | diag | "Último stream —": el discovery por enumeración falla con accessors GPB `@dynamic`. Reescrito con `class_getInstanceMethod` (dispara el resolver) + módulo `OpusLockDiag` con sección DIAGNÓSTICO visible + inventario HAM* | Run verde 33832699696, dylib ~124KB |
| 2026-09-04 | fix-domain | freeze | Congelamiento al reproducir+abrir player. Validado estático con LIEF (`objc_selectors.py`): selectores SÍ compilados (teoría `@dynamic` descartada). Sospechoso nº1: mutar el array GPB vivo. Build bisección: reorder OFF, swizzle seguro solo-directo, toggles por hook, HAM lazy | Run verde 33833948032, dylib ~125KB |
